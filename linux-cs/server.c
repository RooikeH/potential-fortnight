...
{  
    ...
    my_addr.sin_addr.s_addr = INADDR_ANY; //inet_addr = "0.0.0.0" 
...
{  
    ...
    char *buf, *read_addr, *write_addr, *temp, *time_str, *login, *login_sure, *key, *end; //需要用到的缓冲区  
    ...
    {  
        ...
        */  
        ...
        login_sure = (char *)malloc(20);
        memset(login_sure, 0, 20);
        memset(login, 0, 20);
        if ((recvbytes = recv(clientfd, login, 20, 0)) <= 0)  
        {  
            perror("fail to recv");  
            exit(1);  
        }
        ...
        while(fgets(login_sure, 20, stream)){
            if(strcmp(login, login_sure) == 0){
                key = "yes";
                if ((sendbytes = send(clientfd, key, 3, 0)) == -1)  
                {  
                    perror("fail to send");  
                    exit(1);  
                } 
